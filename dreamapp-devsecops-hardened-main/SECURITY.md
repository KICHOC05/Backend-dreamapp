...
A release must pass backend compilation/tests, TypeScript compilation, `npm audit`, `pnpm audit`, Android lint, CodeQL, Gitleaks (secret scan) and Trivy (vulnerabilities/misconfiguration). Production Android builds must use HTTPS/WSS and release minification. An SBOM (SPDX) is generated by the pipeline and must accompany production releases.
